...
def compare_anchors_metrics(anchor: List[VariantData], test: List[VariantData], dist:Metric, rate=Metric.BITRATE, anchor_label="anchor", test_label="test", sanitize=True, log_rate_plot=False, overlap=False) -> Tuple[Figure, int, Any, Any, Any, Any]:
    ...
    return bd_rate_plot(*anchor_metrics, *test_metrics, sanitize=sanitize, anchor_label=anchor_label, test_label=test_label, quality_label=q_key, log_rate_plot=log_rate_plot, overlap=overlap)
...
def bd_rate_plot(R1, DIST1, R2, DIST2, anchor_label="anchor", test_label="test", quality_label="metric", bitrate_unit = "kbit/s", sanitize=False, log_rate_plot=False, overlap=False) -> Tuple[Figure, int, Any, Any, Any, Any]:
...
@click.option('--overlap', required=False, default=False, is_flag=True, help="output csv file to provide overlap rather than bd rate.")
...
def main(working_dir:str, overlap:bool, log_rate_plot:bool, s:bool, anchor_key:str, test_key:str, metric_keys:Tuple[str]):
    """
    ...
    if s:
        ...
        for (seqid, r) in compare_anchors(anchors, tests, metrics, save_plots=True, log_rate_plot=log_rate_plot, overlap=overlap):
            r['reference'] = seqid
            data.append(r)
        ext = "overlap.csv" if overlap else "csv" 
